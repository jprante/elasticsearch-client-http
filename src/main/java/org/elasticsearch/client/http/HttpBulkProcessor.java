 ...
 *
 */
public class HttpBulkProcessor implements Flushable, Closeable {
    ...
    public static Builder builder(Settings settings, ElasticsearchClient client, Listener listener) {
        return new Builder(settings, client, listener);
    }
    ...
    /**
     * Closes the processor. If flushing by time is enabled, then it's shutdown.
     * Any remaining bulk actions are flushed.
     * <p>
     * If concurrent requests are not enabled, returns {@code true} immediately.
     * If concurrent requests are enabled, waits for up to the specified timeout for all bulk requests
     * to complete then returns {@code true},
     ...
     * @return {@code true} if all bulk requests completed and {@code false} if the waiting time elapsed
     * before all the bulk requests completed
    ...
    public HttpBulkProcessor add(BytesReference data, @Nullable String defaultIndex,
                                 @Nullable String defaultType) throws Exception {
        return add(data, defaultIndex, defaultType, null);
    }

    public synchronized HttpBulkProcessor add(BytesReference data, @Nullable String defaultIndex,
    ...
    private boolean isOverTheLimit() {
        return bulkActions != -1 && bulkRequest.numberOfActions() >= bulkActions || bulkSize != -1 &&
    ...
    public interface Listener {
         ...
         * processing has been cancelled externally, the thread's interruption status has been restored prior
    ...
    public static class Builder {
        ...
        /**
         * Creates a builder of bulk processor with the listener that will be used
         ...
         */
        public Builder(Settings settings, ElasticsearchClient client, Listener listener) {
        ...
        public HttpBulkProcessor build() {
            return new HttpBulkProcessor(settings, client, listener, name, concurrentRequests,
                    bulkActions, bulkSize, flushInterval);
        }
    }
